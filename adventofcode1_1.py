...
# Surely, Christmas will go on without you. The tropical island has its own currency and is entirely cash-only. The gold
# coins used there have a little picture of a starfish; the locals just call them stars. None of the currency exchanges
# seem to have heard of them, but somehow, you'll need to find fifty of these coins by the time you arrive so you
# can pay the deposit on your room. To save your vacation, you need to get all fifty stars by December 25th.
#
...
from aocd import data  # string

data_list = data.split()
data_list = list(map(int, data_list))

for index1 in range(0, len(data_list)):
    for index2 in range(0, len(data_list)):
        if index1 != index2:
            #print(data_list[index1], "+", data_list[index2], "=", data_list[index2] + data_list[index1])
            if data_list[index1] + data_list[index2] == 2020:
                print(data_list[index1] * data_list[index2])
...
# together produces the answer, 241861950. In your expense report, what is the product of the three entries that sum to
# 2020?

for a in range(0, len(data_list)):
    for b in range(0, len(data_list)):
        for c in range(0, len(data_list)):
            if a != b and a != c and b != c:
                if data_list[a] + data_list[b] + data_list [c] == 2020:
                    print(data_list[a] * data_list[b] * data_list [c])